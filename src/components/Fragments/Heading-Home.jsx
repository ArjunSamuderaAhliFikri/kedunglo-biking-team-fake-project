...
export default function HeadingHome() {
  ...
  if (status) {
    classButton =
      "transition-all text-white duration-300 absolute rounded-bl-lg rounded-br-lg px-3 w-20 -bottom-32 bg-slate-400 left-1/2 -translate-x-1/2 outline-none";
    classButtonSection =
      "transition-all duration-500 w-full h-28 bg-yellow-500";
  } else {
    classButton =
      "transition-all text-white duration-300 absolute rounded-bl-lg rounded-br-lg px-3 w-20 -bottom-12 bg-emerald-500 left-1/2 -translate-x-1/2 outline-none";
  ...
  return (
    <section className="w-full bg-yellow-500 flex flex-col mx-auto rounded-bl-[30px] rounded-br-[30px] overflow-hidden">
      ...
      <div className="flex flex-row justify-center items-center w-full px-5 mt-5">
        <input
          maxLength={20}
          className="w-full outline-none rounded-tl-md rounded-bl-md py-[6px] px-3"
          type="text"
          placeholder="Cari..."
        />
